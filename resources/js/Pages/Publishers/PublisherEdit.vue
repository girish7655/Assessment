...
// Page Props and publisher Data
const page = usePage();
const publisher = computed(() => page.props.publisher); // Retrieve publisher details

// Form state with pre-filled publisher name
const form = useForm({
...
const validateForm = () => {
  ...
  if (!form.name.trim()) {
    nameError.value = 'Publisher name cannot be empty.';
...
// Handle Form Submission (Update publisher)
const submitForm = () => {
    if (validateForm()) {
        // Clear success/error messages before submission
        page.props.flash.success = '';
        page.props.errors.name = '';
        form.put(route('publishers.update', publisher.value.id)); // PUT request to update publisher
    }
};

// Handle Cancel Button Click (Redirect to publishers list)
const cancelForm = () => {
    router.get(route('publishers.index')); // Redirect back to publishers List
};
</script>
<template>
    <AuthenticatedLayout>
